 ...
 *
 * Endpoints inexistentes no backend real (estatísticas, análise de IA) são
 * COMPUTADOS localmente a partir da lista real de notas + configuração, para
 * não depender de rotas que o serviço não implementa.
 */
...
// ─── Estatísticas / Análise IA (computadas localmente) ──────────────────────────

function computarEstatisticas(nfs: NotaFiscal[]): EstatisticasFiscais {
  const now = Date.now()
...
/** Busca todas as notas (paginação máxima) para os cálculos locais. */
...
export const fiscalService = {
  ...
  /** Estatísticas computadas localmente a partir das notas reais. */
  async estatisticas(): Promise<EstatisticasFiscais> {
    const nfs = await buscarTodasNotas()
    return computarEstatisticas(nfs)
  },
